...
@Component({
  ...
  template: `
    <div class="form-page">
      <h2>{{ isEdit() ? 'Edit Sales Invoice' : 'New Sales Invoice' }}</h2>
      ...
      @if (loading()) {
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input formControlName="placeOfSupply" class="form-control" />
              </div>
              <div class="form-group">
                <label>
          ...
          <div class="form-actions">
            ...
            <button type="button" class="btn btn-secondary" routerLink="/transactions/sales">Cancel</button>
          </div>
        </form>
      }
    </div>
...
export class SalesInvoiceFormComponent implements OnInit {
  ...
  loadInvoice(id: number): void {
    ...
    this.api.getSalesInvoiceById(id).subscribe({
      next: (inv) => {
        this.form.patchValue({
          invoiceNumber: inv.invoiceNumber,
          invoiceDate: inv.invoiceDate?.toString().substring(0, 10),
          customerId: inv.customerId,
          placeOfSupply: (inv as any).placeOfSupply,
  ...
  onItemSelect(index: number): void {
    ...
    if (item) {
      line.patchValue({ rate: item.sellingRate ?? 0, gstRate: item.gstRate?.rate ?? 0 });
  ...
  onSubmit(): void {
    ...
    const request = id
      ? this.api.updateSalesInvoice(+id, data)
      : this.api.createSalesInvoice(data);

    request.subscribe({
      next: () => { this.saving.set(false); this.router.navigate(['/transactions/sales']); },
