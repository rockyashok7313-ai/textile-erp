...
export class PurchaseInvoiceListComponent implements OnInit {
  ...
  loadInvoices(): void {
    ...
    this.api.getAllPurchaseInvoices().subscribe({
      next: (data) => {
  ...
  filterInvoices(): void {
    ...
    if (this.searchTerm) {
      ...
      result = result.filter(i =>
        ...
        (i.supplierInvoiceNumber ?? '').toLowerCase().includes(term)
      );
    }
  ...
  postInvoice(inv: PurchaseInvoice): void {
    if (confirm(`Post invoice ${inv.invoiceNumber}?`)) {
      this.api.postPurchaseInvoice(inv.id).subscribe(() => this.loadInvoices());
    }
  }

  deleteInvoice(inv: PurchaseInvoice): void {
    if (confirm(`Delete invoice ${inv.invoiceNumber}?`)) {
      this.api.deletePurchaseInvoice(inv.id).subscribe(() => this.loadInvoices());
    }
  }
}
